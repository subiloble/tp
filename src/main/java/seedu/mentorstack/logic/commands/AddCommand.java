...
public class AddCommand extends Command {
    ...
    public AddCommand(Person person) {
        ...
        toAdd = person;
        logger.log(Level.INFO, "AddCommand created with person: {0}", person);
    }

    @Override
    public CommandResult execute(Model model) throws CommandException {
        requireNonNull(model);
        logger.log(Level.INFO, "Executing AddCommand for person: {0}", toAdd);

        if (model.hasPerson(toAdd)) {
            logger.log(Level.WARNING, "Duplicate person detected: {0}", toAdd);
            throw new CommandException(MESSAGE_DUPLICATE_PERSON);
        }

        model.rememberMentorstack(); // save the state for undo
        model.addPerson(toAdd);
        logger.log(Level.INFO, "Person successfully added: {0}", toAdd);
